...
test('Ctrl+click disconnect removes the exact cable', () => {
  ...
  assert.equal(deleteConnection(hub.network, cable), true);
...
test('plain click does not disconnect', () => {
  ...
  assert.equal(hub.network.connections().length, 1);
  assert.equal(buildVisualConnections(hub.network).length, 1);
});
...
test('unplugging one branch preserves other fan-out connections', () => {
  ...
  const bConn = hub.network.connectionsTo('b', 'i')[0];
  assert.equal(deleteConnection(hub.network, bConn), true);
  assert.equal(hub.network.connections().length, 1);
  const remaining = hub.network.connections()[0];
  assert.equal(remaining.to.nodeId, 'c', 'fan-out branch to c must remain');
});

test('network persistence updates after unplug', () => {
  const hub = makeFullHub();
  seed(hub);
  createConnection(hub.network, { nodeId: 'a', portId: 'o' }, { nodeId: 'b', portId: 'i' });
  assert.equal(hub.settings.get('networkConnections').length, 1);
  const cable = buildVisualConnections(hub.network)[0];
  deleteConnection(hub.network, cable);
  assert.deepEqual(hub.settings.get('networkConnections'), []);
});
...
test('disconnect leaves node state and layout unchanged', async () => {
  ...
  seed(hub);
  createConnection(hub.network, { nodeId: 'a', portId: 'o' }, { nodeId: 'b', portId: 'i' });
  hub.settings.set('networkLayout', { a: { x: 100, y: 100 }, b: { x: 400, y: 100 } });
  const beforeLayout = hub.settings.get('networkLayout');

  const cable = buildVisualConnections(hub.network)[0];
  deleteConnection(hub.network, cable);

  assert.deepEqual(hub.settings.get('networkLayout'), beforeLayout, 'layout unchanged');
  assert.ok(hub.network.getNode('a') && hub.network.getNode('b'), 'nodes remain');
});
...
test('cable creation still works', () => {
  const hub = makeFullHub();
  seed(hub);
  const result = createConnection(hub.network, { nodeId: 'a', portId: 'o' }, { nodeId: 'b', portId: 'i' });
