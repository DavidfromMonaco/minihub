...
test('track fader and mute update live DSP without rebuilding or panicking the arrangement', async () => {
  const { controller, commands } = rig();
  const track = controller.model.addTrack('midi');
  controller.changed();
  await new Promise((resolve) => queueMicrotask(resolve));
  const syncs = commands.filter((command) => command.type === 'syncSequencer').length;
  controller.setTrackControl(track.id, { volume: 0.501187 }, { render: false });
  controller.setTrack(track.id, { muted: true });
  await new Promise((resolve) => queueMicrotask(resolve));
  assert.equal(commands.filter((command) => command.type === 'syncSequencer').length, syncs);
  assert.deepEqual(commands.filter((command) => command.type === 'trackControl').slice(-2), [
    { type: 'trackControl', trackId: track.id, gain: 0.501187, muted: false },
    { type: 'trackControl', trackId: track.id, gain: 0.501187, muted: true }
  ]);
});
