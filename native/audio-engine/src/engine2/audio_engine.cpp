...
bool AudioEngine::openDefault(double sampleRate, std::uint32_t frames,
                              bool enableInput, std::string& error)
{
    return selectDevice({}, sampleRate, frames, enableInput, error);
}

bool AudioEngine::selectDevice(const std::string& outputName, double sampleRate,
                               std::uint32_t frames, bool enableInput,
                               std::string& error)
{
    stop();
    if (!device_.open(outputName, sampleRate, frames, enableInput, error))
        return false;
