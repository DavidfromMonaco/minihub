...
public:
    ...
    struct TrackSignalTrace {
        std::string trackId, trackType, destinationBuffer;
        int activeClips = 0;
        float peakBeforeSum = 0, peakAfterSum = 0;
        float gainApplied = 1, peakAfterGain = 0;
    };
    ...
    /** Publish the instrument boundary measured inside Chain::processBlock. */
    void observeMidiTrackGain(const std::string& outputId, const Transport&,
                              float peakBeforeGain, float gainApplied,
                              float peakAfterGain) noexcept;
...
private:
    ...
    struct TrackRuntime {
        ...
        std::atomic<float> gainApplied { 1.0f };
        std::atomic<float> peakAfterGain { 0.0f };
    };
    struct Track {
