                           ...
                           juce::Array<juce::var>& audioInfo, std::string& error)
{
    ...
    for (const auto& value : *tracks) {
        ...
        track.muted=value["muted"].isBool()?(bool)value["muted"]:false;
        track.volume=boundedGain(value["volume"]);
...
{
    if(!transport.processingPlaying())return;const bool exportContext=&transport==&offlineExportTransport_;auto* plan=acquirePlan(exportContext);if(!plan)return;
    const double start=transport.ppqPosition(),qps=transport.quarterNotesPerSample();const bool chase=(exportContext?needsExportChase_:needsChase_).exchange(false);
    const bool sourceEnded=exportContext&&start>=exportSourceEndPpq();
    const bool sourceStopsThisBlock = exportContext
    ...
    for(auto& track:plan->tracks){if(track.type!="midi"||track.muted||track.outputId.empty())continue;auto& buffer=track.midiScratch;buffer.clear();
        ...
        if(buffer.isEmpty())continue;if(track.midiOutputKind==Track::MidiOutputKind::physical){if(hardware)hardware->sendBlock(buffer,callbackStartMs,sampleRate_);}else if(track.midiOutputKind==Track::MidiOutputKind::processor){if(midiPlan)midiPlan->pushInputBuffer(track.outputId,buffer);}else if(track.destination)track.destination->pushMidi(buffer);
    }
...
void SequencerEngine::renderAudioForOutput(juce::AudioBuffer<float>& out,int count,Transport& transport,const std::string& outputId) noexcept
{
    ...
    for(const auto& track:plan->tracks){if(track.type!="audio"||track.muted||(!outputId.empty()&&track.outputId!=outputId))continue;for(const auto& clip:track.audio){if(!clip.asset)continue;for(int sample=0;sample<count;++sample){const double q=transport.ppqAtSample(sample);if(exportContext&&q>=exportSourceEndPpq())continue;if(q<clip.startPpq||q>=clip.startPpq+clip.lengthPpq)continue;const double seconds=clip.trimStartSeconds+(q-clip.startPpq)*60.0/bpm;if(seconds<clip.trimStartSeconds||seconds>=clip.trimEndSeconds)continue;const double source=seconds*clip.asset->sampleRate;const int i=(int)source;if(i<0||i+1>=clip.asset->samples.getNumSamples())continue;const float f=(float)(source-i),gain=track.volume*clip.gain;for(int ch=0;ch<2;++ch){const float* data=clip.asset->samples.getReadPointer(ch);out.addSample(ch,sample,(data[i]+(data[i+1]-data[i])*f)*gain);}}}}
    releasePlan();
}
...
{
    needsChase_.store(true);auto* plan=activePlan_.load(std::memory_order_acquire);if(!plan)return;for(auto& track:plan->tracks)if(track.destination)track.destination->panic();
}

void SequencerEngine::panicExport() noexcept
{
    needsExportChase_.store(true);auto* plan=exportPlan_.load(std::memory_order_acquire);if(!plan)return;for(auto& track:plan->tracks)if(track.destination)track.destination->panic();exportCleanupPending_.store(true,std::memory_order_release);
}
    ...
    std::string& error)
{
    ...
    for(const auto& original:source->tracks){Track track;track.id=original.id;track.type=original.type;track.inputId=original.inputId;track.outputId=original.outputId;track.armed=original.armed;track.muted=original.muted;track.volume=original.volume;track.midiOutputKind=original.midiOutputKind;track.midi=original.midi;track.audio=original.audio;track.clips=original.clips;
                                  ...
                                  juce::String& error)
{
    ...
    exportCleanupPending_.store(false,std::memory_order_release);exportPlan_.store(preparedExportPlan_.get(),std::memory_order_release);
