...
void ArpeggiatorRuntime::flush(const std::vector<MidiDestination>& dest,MidiOutputSink* hardware,double startMs,double sampleRate) noexcept {if(output_.isEmpty())return;bool hardwareUsed=false;for(const auto& d:dest){if(d.kind==MidiDestinationKind::physicalOutput)hardwareUsed=true;else if(d.chain)d.chain->pushMidi(output_);}if(hardwareUsed&&hardware)hardware->sendBlock(output_,startMs,sampleRate);}
void ArpeggiatorRuntime::panic(const std::vector<MidiDestination>& dest,MidiOutputSink* hardware) noexcept {output_.clear();bool hardwareUsed=false;for(auto& a:active_)if(a.on){emit(juce::MidiMessage::noteOff(a.channel,a.note),0);a.on=false;}for(int channel=1;channel<=16;++channel){emit(juce::MidiMessage::allNotesOff(channel),0);emit(juce::MidiMessage::allSoundOff(channel),0);}for(const auto& d:dest){if(d.kind==MidiDestinationKind::physicalOutput)hardwareUsed=true;else if(d.chain){d.chain->panic();d.chain->pushMidi(output_);}}if(hardwareUsed&&hardware)hardware->panic();}
void ArpeggiatorRuntime::process(int samples,Transport& t,const std::vector<MidiDestination>& dest,MidiOutputSink* hardware,double startMs,double sampleRate,const juce::MidiBuffer* scheduledInput) noexcept {
 ...
 if(!t.processingPlaying()){if(wasPlaying_)panic(dest,hardware);wasPlaying_=false;lastStep_=std::numeric_limits<int64_t>::min();return;} wasPlaying_=true;
...
std::unique_ptr<MidiExecutionPlan> MidiExecutionPlan::compile(const MidiGraphSpec& spec,const std::function<Chain*(const std::string&)>& lookup,std::string& error){auto p=std::make_unique<MidiExecutionPlan>();std::unordered_set<std::string> ids;for(const auto&s:spec.nodes){if(s.kind!="arpeggiator")continue;if(!ids.insert(s.id).second){error="duplicate MIDI node";return{};}Node n;n.id=s.id;n.arp=std::make_unique<ArpeggiatorRuntime>(s.arp);n.scheduledInput.ensureSize(8192);for(const auto&d:s.destinations){if(isPhysicalMidiDestination(d))n.destinations.push_back({MidiDestinationKind::physicalOutput,nullptr});else{auto*c=lookup(d);if(!c){error="unknown MIDI destination: "+d;return{};}n.destinations.push_back({MidiDestinationKind::chain,c});}}p->nodes_.push_back(std::move(n));}return p;}
