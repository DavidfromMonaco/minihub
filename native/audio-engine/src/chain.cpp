                         ...
                         float* peakAfterInstrumentGain)
{
    ...
    for (auto& owned : plugins_)
    {
        ...
        if (p->isInstrument() && !midiConsumed)
        {
            ...
            const float before = std::max(buffer.getMagnitude(0, 0, buffer.getNumSamples()),
                                          buffer.getNumChannels()>1
                                              ? buffer.getMagnitude(1, 0, buffer.getNumSamples()) : 0.0f);
            ...
            if (peakAfterInstrumentGain)
                *peakAfterInstrumentGain = std::max(buffer.getMagnitude(0, 0, buffer.getNumSamples()),
                                                     buffer.getNumChannels()>1
                                                         ? buffer.getMagnitude(1, 0, buffer.getNumSamples()) : 0.0f);
        }
        else
