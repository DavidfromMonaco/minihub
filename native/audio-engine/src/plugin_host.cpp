...
public:
    ...
    bool prepare(double sampleRate, int blockSize, bool offline, std::string& error)
    {
        ...
        size_t channelSlots = 0;
        for (Steinberg::int32 bus = 0; bus < processData_.numInputs; ++bus)
            channelSlots += static_cast<size_t>(processData_.inputs[bus].numChannels);
        for (Steinberg::int32 bus = 0; bus < processData_.numOutputs; ++bus)
            channelSlots += static_cast<size_t>(processData_.outputs[bus].numChannels);
        scratchChannels_.assign(channelSlots,
    ...
    bool process(juce::AudioBuffer<float>& audio, const juce::MidiBuffer& midi,
                 juce::AudioPlayHead* playHead) noexcept
    {
        const auto frames = audio.getNumSamples();
        if (!processing_ || frames <= 0 || frames > maximumBlockSize_
        ...
        if (processData_.numOutputs > 0
            && processData_.outputs[0].numChannels == 1
            && audio.getNumChannels() > 1)
            audio.copyFrom(1, 0, audio, 0, 0, frames);
...
private:
    ...
    bool configureBuses(std::string& error)
    {
        ...
        // VST3 requires one arrangement entry per exposed bus.  Sending only
        // the main bus leaves wrappers with fixed auxiliary-bus counts unable
        // to apply the requested layout.  Explicit kEmpty entries negotiate
        // every auxiliary bus off while preserving MiniHub's stereo contract.
        const auto tryArrangement = [&](SpeakerArrangement main)
        {
            ...
            std::vector<SpeakerArrangement> outputs(static_cast<size_t>(outputCount),
                                                     SpeakerArr::kEmpty);
            if (!inputs.empty()) inputs[0] = main;
            outputs[0] = main;
            return succeeded(processor_->setBusArrangements(
                inputs.empty() ? nullptr : inputs.data(), inputCount,
                outputs.data(), outputCount));
        };
        const bool arranged = tryArrangement(SpeakerArr::kStereo)
                           || tryArrangement(SpeakerArr::kMono);
        if (!arranged)
        {
            error = className_ + " rejected a MiniHub-compatible mono/stereo layout";
            return false;
        }

        for (Steinberg::int32 index = 0; index < outputCount; ++index)
            component_->activateBus(kAudio, kOutput, index, index == 0);
        for (Steinberg::int32 index = 0; index < inputCount; ++index)
            component_->activateBus(kAudio, kInput, index, index == 0);
        ...
        inputChannels_ = inputCount > 0
            && succeeded(processor_->getBusArrangement(kInput, 0, arrangement))
            ? SpeakerArr::getChannelCount(arrangement) : 0;
        arrangement = SpeakerArr::kEmpty;
        outputChannels_ = succeeded(processor_->getBusArrangement(kOutput, 0, arrangement))
            ? SpeakerArr::getChannelCount(arrangement) : 0;
        if (outputChannels_ < 1 || outputChannels_ > 2 || inputChannels_ > 2)
        {
    ...
    void bindBuffers(juce::AudioBuffer<float>& audio) noexcept
    {
        const auto frames = static_cast<size_t>(audio.getNumSamples());
        size_t scratch = 0;
        for (Steinberg::int32 busIndex = 0; busIndex < processData_.numInputs; ++busIndex)
        {
            auto& bus = processData_.inputs[busIndex];
            bus.silenceFlags = 0;
            for (Steinberg::int32 channel = 0; channel < bus.numChannels; ++channel)
            {
                auto& storage = scratchChannels_[scratch++];
                const int source = std::min<int>(channel, audio.getNumChannels() - 1);
                std::copy_n(audio.getReadPointer(source), frames, storage.data());
                bus.channelBuffers32[channel] = storage.data();
            }
        }
        for (Steinberg::int32 busIndex = 0; busIndex < processData_.numOutputs; ++busIndex)
        {
            auto& bus = processData_.outputs[busIndex];
            bus.silenceFlags = 0;
            for (Steinberg::int32 channel = 0; channel < bus.numChannels; ++channel)
            {
                auto& storage = scratchChannels_[scratch++];
                if (busIndex == 0 && channel < audio.getNumChannels())
                {
                    auto* output = audio.getWritePointer(channel);
                    std::fill_n(output, frames, 0.0f);
                    bus.channelBuffers32[channel] = output;
                }
                else
                {
                    std::fill_n(storage.data(), frames, 0.0f);
                    bus.channelBuffers32[channel] = storage.data();
                }
            }
        }
    ...
    void convertMidi(const juce::MidiBuffer& midi) noexcept
    {
        using namespace Steinberg::Vst;
        for (const auto metadata : midi)
        {
            ...
            Event event {};
            event.busIndex = 0;
            event.sampleOffset = std::clamp(metadata.samplePosition, 0, maximumBlockSize_ - 1);
...
{
    if (!plugin_ || !isReady_ || !beginRealtimeRead())
        return;
    ...
    const bool processed = plugin_->process(buffer, midi, assignedPlayHead_);
    if (!processed)
        buffer.clear();
