...
struct Vst3BufferProcessTrace {
    uint64_t blockId = 0;
    uint32_t processCallInBlock = 0;
    int numSamples = 0;
    int inputBusCount = 0;
    ...
    bool copiedToPluginInstance = false;
};
...
public:
    ...
    void processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi,
                      int numSamples, uint64_t blockId) noexcept;
    ...
    Vst3BufferProcessTrace vst3BufferProcessTrace() const noexcept;
...
private:
    ...
    std::atomic<uint64_t> vst3BlockId_ {0};
    std::atomic<uint32_t> vst3ProcessCallInBlock_ {0};
    std::atomic<int> vst3NumSamples_ {0};
    std::atomic<int> vst3InputBusCount_ {0}, vst3OutputBusCount_ {0};
    std::atomic<int> vst3MainInputBus_ {-1}, vst3MainOutputBus_ {-1};
    std::atomic<int> vst3MainInputChannels_ {0}, vst3MainOutputChannels_ {0};
    std::atomic<std::uintptr_t> vst3InputLeft_ {0}, vst3InputRight_ {0};
    std::atomic<std::uintptr_t> vst3OutputLeft_ {0}, vst3OutputRight_ {0};
    std::atomic<bool> vst3InputOutputDistinct_ {true};
    std::atomic<bool> vst3CopiedToPluginInstance_ {false};
