...
private:
    ...
    void sendEditorBounds(PluginInstance& inst, bool raised);
    ...
    // The WASAPI capture endpoint joins the duplex stream only when an
    // `audio-input` node exists. The Windows default capture device is very
    // often a different physical unit than the render device (USB codec vs
    // onboard), and one PortAudio stream spanning two independent word clocks
    // drifts and drops samples periodically.
    bool audioInputRequested_ = false;
    ...
    // Plugin state capture freezes the plugin for the audio thread (see
    // PluginInstance::beginControlMutation). It is therefore only allowed once
    // the output has been provably inaudible for several telemetry ticks.
    int quiescentTelemetryTicks_ = 0;
    // Previously published drop totals, so the telemetry can report a per
    // window delta as well as the run total.
    std::uint64_t lastChainBlocksSkipped_ = 0;
    std::uint64_t lastPluginBlocksSkipped_ = 0;
    static constexpr int kQuiescentTicksBeforeStateCapture = 3;
    static constexpr float kQuiescencePeak = 1.0e-4f; // about -80 dBFS
    ...
    // A gap between callback starts only risks starvation once it exceeds the
    // audio actually buffered ahead in the device, which is what
    // Pa_GetStreamInfo reports as output latency. Comparing it to one block
    // deadline counted PortAudio's normal burst delivery as an underrun.
    double callbackGapThresholdMilliseconds_ = 0.0;
