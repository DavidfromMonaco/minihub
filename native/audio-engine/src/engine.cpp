...
void Engine::cmdSyncMidiNetwork(const juce::var& msg)
{
    ...
    const auto synced=[this,&spec](bool rebuilt){juce::var out=makeObject();setProp(out,"type","midiNetworkSynced");setProp(out,"nodeCount",(int)spec.nodes.size());setProp(out,"rebuilt",rebuilt);ipc_.send(out);};
    // Same nodes, same cables: only values changed -- a rate, a step's pitch,
    // sent by a knob or by a plugin's sequence every beat. The running
    // arpeggiators take them and keep the notes they hold. A rebuild starts them
    // empty, and its panic silences every chain, a pad played by a track included.
    if(auto*running=activeMidiPlan_.load(std::memory_order_acquire);running&&MidiExecutionPlan::sameWiring(activeMidiSpec_,spec)){running->setValues(spec);activeMidiSpec_=spec;synced(false);return;}
    std::string error;auto plan=MidiExecutionPlan::compile(spec,[this](const std::string&id){return getOrCreateChain(juce::String(id));},error);if(!plan){reject(juce::String(error));return;}panicAllMidi();activeMidiSpec_=spec;auto*published=plan.get();midiPlans_.push_back(std::move(plan));activeMidiPlan_.store(published,std::memory_order_release);synced(true);
}
