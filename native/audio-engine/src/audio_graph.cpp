    ...
    bool pdcEnabled)
{
    ...
    for(int original:order){const auto& s=spec.nodes[(size_t)original];Node n;n.id=s.id;n.kind=s.kind;n.masterLevel=std::clamp(s.masterLevel,0.0f,2.0f);n.stepCount=s.stepCount;n.steps=s.steps;n.diagnosticCyclesPerSample=s.diagnosticCyclesPerSample;n.diagnosticAmplitude=std::clamp(s.diagnosticAmplitude,-2.0f,2.0f);n.diagnosticStartSample=std::max<int64_t>(0,s.diagnosticStartSample);n.diagnosticEndSample=std::max(n.diagnosticStartSample,s.diagnosticEndSample);
        ...
        for(const auto& in:s.inputs){const int source=compiled.at(index.at(in.sourceNodeId));n.sources.push_back(source);n.levels.push_back(std::clamp(in.level,0.0f,2.0f));n.mutes.push_back(in.muted);maximumSourceLatency=std::max(maximumSourceLatency,plan->nodes_[(size_t)source].latencySamples);}
                                 ...
                                 const juce::AudioBuffer<float>* hardwareInput) noexcept
{
    ...
    for(auto& n:nodes_){n.output.clear(0,count);
        ...
        else if(n.kind==AudioNodeKind::mixer){for(size_t i=0;i<n.sources.size();++i)if(!n.mutes[i])for(int ch=0;ch<2;++ch)n.output.addFrom(ch,0,sourceBuffer(i),ch,0,count,n.levels[i]*n.masterLevel);if(n.signalMeter){n.signalMeter->observe(n.output,count,AudioSignalBoundary::input);n.signalMeter->observe(n.output,count,AudioSignalBoundary::output);}}
