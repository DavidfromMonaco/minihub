...
{
    const juce::File dexed("C:\\Program Files\\Common Files\\VST3\\Dexed.vst3"),vital("C:\\Program Files\\Common Files\\VST3\\Vital.vst3");expect(dexed.exists()&&vital.exists(),"Dexed and Vital VST3 bundles are installed for cross-track isolation");if(!dexed.exists()||!vital.exists())return;const auto dexedRecords=mlh::Vst3Scanner::scanFile(dexed.getFullPathName()),vitalRecords=mlh::Vst3Scanner::scanFile(vital.getFullPathName());expect(!dexedRecords.empty()&&!vitalRecords.empty(),"Dexed and Vital scan for cross-track isolation");if(dexedRecords.empty()||vitalRecords.empty())return;testHostedVstPairIsolation(dexedRecords[0],dexedRecords[0],"Dexed/Dexed",false);testHostedVstPairIsolation(dexedRecords[0],vitalRecords[0],"Dexed/Vital",false);testHostedVstPairIsolation(vitalRecords[0],vitalRecords[0],"Vital/Vital",false);
}
