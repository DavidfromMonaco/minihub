const [port = '9462', pluginPath = ''] = process.argv.slice(2);
if (!pluginPath) throw new Error('Usage: runtime-commercial-vst-gain-gauntlet.mjs <port> <vst3-path>');
...
const expression = `(async () => {
  ...
  try {
    for (const target of [
      { chainId: chainA, instanceId: instanceA, requestId: 'commercial-load-a' },
      { chainId: chainB, instanceId: instanceB, requestId: 'commercial-load-b' }
    ]) {
    ...
    return {
      verdict: 'PASS', pluginId, heldSeconds: 20.5,
