 ...
 * network. The network remains authoritative:
 *
...
/** Everything that defines the SHAPE of the audio network.
 *
 ...
 *  emits one `input` per pixel of a drag; routing those through syncAudioNetwork
 *  recompiled the network tens of times per second and rebuilt every SourceDelay,
...
export function buildRoutingSync(hub) {
  ...
  // so a value edit never escalates into a network recompile.
  let publishedTopology = '';
  let publishedValues = '';
  let midiNetworkPublished = '';
  const midiPublished = new Map();

  const sync = () => {
    if (!hub.engine) return;
    for (const node of hub.network.listNodes()) if (node.type === 'vst') {
      const enabled=hub.network.connectionsTo(node.id, 'midi-in').length > 0;
      if(midiPublished.get(node.id)!==enabled){midiPublished.set(node.id,enabled);hub.engine.setChainMidiEnabled(node.id,enabled);}
    }

    const nodes = describeAudioNetwork(hub);
    const topology = audioTopologyKey(nodes);
    const values = JSON.stringify(audioNodeValues(nodes));
    if (topology !== publishedTopology) {
      // syncAudioNetwork carries the values too, so they are never left behind.
      publishedTopology = topology;
      publishedValues = values;
      hub.diagnostics?.log(`startup:audio-network-sync nodes=${nodes.length}`);
      hub.engine.syncAudioNetwork(nodes);
    } else if (values !== publishedValues) {
      publishedValues = values;
      hub.engine.setAudioNodeValues(audioNodeValues(nodes));
    }

    const midiNodes=describeMidiNetwork(hub); const midiKey=JSON.stringify(midiNodes);
    if(midiKey!==midiNetworkPublished){
      midiNetworkPublished=midiKey;
      hub.engine.syncMidiNetwork(midiNodes);
    }
  };

  /** Drop the cache so the next sync re-publishes everything (engine restart). */
  sync.reset = () => { publishedTopology=''; publishedValues=''; midiNetworkPublished=''; midiPublished.clear(); };
  return sync;
}

export function setupEngineSync(hub) {
  const sync = buildRoutingSync(hub);
  hub.events.on('network:change', sync);
  ...
  hub.events.on('network:change', () => hub.sequencer?.syncNative());
  ...
  // published so the next sync sends the whole network again.
